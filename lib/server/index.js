...
let sysPort = process.env.PORT || 9090;
...
module.exports.start = function () {
	...
	log.trace(`Attempting to listen on ${config.host}:${sysPort}`);
	server.on('error', err => {
		switch (err.code) {
			case 'EADDRINUSE':
				...
				log.error(`[server.js] Listen ${config.host}:${sysPort} in use, is NodeCG already running? NodeCG will now exit.`);
				break;
			default:
	...
	server.listen({
		host: config.host,
		port: process.env.NODECG_TEST ? undefined : sysPort
	}, () => {
		if (process.env.NODECG_TEST) {
