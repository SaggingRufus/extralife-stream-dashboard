...
const protocol = process.env.protocol || ((config.ssl && config.ssl.enabled) || config.login.forceHttpsReturn) ? 'https' : 'http';
let sysClientId = process.env.twitchClientId || config.login.twitch.clientID;
let sysClientSecret = process.env.twitchClientSecret || config.login.twitch.clientSecret;
let sysSessionSecret = process.env.sessionSecret || config.login.sessionSecret;
let sysBaseUrl = process.env.baseURL || config.baseURL;
...
if (config.login.steam && config.login.steam.enabled) {
	...
	passport.use(new SteamStrategy({
		returnURL: `${protocol}://${sysBaseUrl}/login/auth/steam`,
		realm: `${protocol}://${sysBaseUrl}/login/auth/steam`,
...
if (config.login.twitch && config.login.twitch.enabled) {
	...
	scope = scope.join(' ');
	log.info(`${protocol}://${sysBaseUrl}/login/auth/twitch`);
	
	passport.use(new TwitchStrategy({
		clientID: sysClientId,
		clientSecret: sysClientSecret,
		callbackURL: `${protocol}://${sysBaseUrl}/login/auth/twitch`,
		scope,
		customHeaders: {'Client-ID': sysClientId}
...
app.use(cookieParser(sysSessionSecret));
app.use(session({
	secret: sysSessionSecret,
	resave: false,
